...
class DRODataset(Dataset):
    def __init__(self, dataset, process_item_fn, n_groups, n_classes, group_str_fn):
      ...
      #  print(group_array.shape)
        self._group_array = torch.LongTensor(group_array); #self._group_array = torch.LongTensor(group_array)
       # print(self._group_array.shape)
        self._y_array = torch.LongTensor(y_array)
        self._group_counts = torch.Tensor(np.sum(group_array, axis = 0))
    ...
    def get_loader(self, train, reweight_groups, **kwargs):
        if not train: # Validation or testing
            ...
            hard_group_array = torch.argmax(self._group_array, dim = -1)
            # print(self._group_counts)
            # print(group_weights)
            weights = group_weights[hard_group_array]
            #print(self._group_array)
          #  print(weights)
