...
class LossComputer:
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        group_map = group_idx
        group_map = torch.t(group_map).reshape(self.n_groups,-1)
        group_count = torch.sum(group_idx, dim = 0)
        group_denom = group_count + (group_count==0).float() 
        group_loss = (group_map @ losses.double().view(-1))/group_denom
    ...
    def update_stats(self, actual_loss, group_loss, group_acc, group_count, weights=None):
        ...
        self.avg_per_sample_loss = group_frac.double() @ self.avg_group_loss
        self.avg_acc = group_frac.double() @ self.avg_group_acc
